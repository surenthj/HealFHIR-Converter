...
function postResJson() {
    var reqBody = outputCode.getValue();

    $.ajax('https://hcsagitecfhirservice.azurewebsites.net/Practitioner/', {
        'data': reqBody,
        'type': 'POST',
        'processData': false,
        ...
        'error': function (response) {
            try {
                ...
                console.error('Error saving output: ' + JSON.stringify(response));
                displayBanner('Error saving output: ' + JSON.stringify(response), "alert-danger");
            }
        }
    });
}
